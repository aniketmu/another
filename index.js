...
app.get("/file/:fileId", async (req, res) => {
  try {
    ...
    Channel.findOne({ "messages.file.id": fileId })
      .then((channel) => {
        if (channel) {
          const foundMessage = channel.messages.find(
...
app.post("/channel", async (req, res) => {
  ...
  try {
    const channel = await Channel.find({
      "members.objectUserId": objectUserId,
    });
    if (!channel.length) {
...
app.post("/add-channel", async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .send({ message: `Channel with name: ${name} created successfully` });
  } catch (error) {
    console.log(error);
  ...
  try {
    ...
    if (
      channel.members.some(
        (member) => member.objectUserId.toString() === user._id.toString()
      ) ||
    ...
    res
      .status(200)
      .send({ message: `User ${user.name} invited to join the channel` });
  } catch (error) {
...
app.post("/chat-message-file", upload.single("file"), (request, response) => {
  ...
  s3.upload(params, (err, data) => {
    ...
    console.log(data);
    response.send({
      message: "Direct message and file details stored.",
      fileObj,
    });
  });
});
